...
def index(request):
    ...
    return render(request, 'products/index.html', {'products': products, 'banners': banners})
...
def search_view(request):
    ...
    if query:
        products = tuple(product for product in Product.objects.all() if query in product.name.lower())
...
def product_detail(request, product_id):
    ...
    cart = request.session.get('cart', {})
    added = str(product.id) in cart  # Ключ всегда строка в сессии

    return render(request, 'products/product_detail.html', {'product': product, 'added': added, 'is_liked': is_liked,})
